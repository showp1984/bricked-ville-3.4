...
static void kgsl_ion_alloc_free(struct kgsl_memdesc *memdesc)
{
	...
	if(memdesc->handle) ion_free(kgsl_client, memdesc->handle);
				...
				size_t size, unsigned int protflags)
{
	...
	KGSL_STATS_ADD(size, kgsl_driver.stats.vmalloc, kgsl_driver.stats.vmalloc);
				...
				size_t size, int flags)
{
	...
	return _kgsl_sharedmem_ion_alloc(memdesc, pagetable, size,
       protflags);
}
